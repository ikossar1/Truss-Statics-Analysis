...
CC
DD
VarSoln=vpa([inv(CC)*DD]');    
VarSoln=vpa(round(1000*VarSoln)/1000);

if rank(CC)<max(size(CC)) 
   display( 'ERROR: Remove zero force members' )
   CC
   DD
   %rref( CC  )
   %rref( [CC double(ddxy)'])
...
scale=.1*abs(xxlim(2)-xxlim(1))/ff_scale;
...
for ii=1:numforces
    ...
    ForcePlot(rrs(ii,:), ffnew_ii(ii,:),scale*(1+rand(1)), mycol, 3);
